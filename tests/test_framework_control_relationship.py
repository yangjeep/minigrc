...
def test_duplicate_control_requirement_mapping_is_impossible_at_db_level(app):
    session_factory = app.state.session_factory
    with session_factory() as session:
        framework = Framework(name="Dup Test Framework", version="1.0", description="")
        session.add(framework)
        session.flush()

        requirement = FrameworkRequirement(
            framework_id=framework.id, reference_code="D.1", title="Dup requirement", summary=""
        )
        session.add(requirement)
        session.flush()

        control = InternalControl(name="Dup control", owner="owner@example.com")
        session.add(control)
        session.flush()
